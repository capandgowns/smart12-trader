...
def login():
    st.title("🚀 Smart12Trader")
    st.subheader("Private Login")
...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        for sym in base_stocks:
            ...
            score = round(70 + (ret1y * 0.3) - (rsi - 50) * 0.5, 1)
    ...
    with tab2:
        ...
        selected = st.multiselect("Select Stocks", [s.replace('.NS','') for s in base_stocks])
        st.info("More filters coming soon...")

    with tab3:
        stock = st.selectbox("Select Stock", [s.replace('.NS','') for s in base_stocks])
        hist, _ = get_data(stock + ".NS")
        if hist is not None:
            fig = go.Figure([go.Candlestick(x=hist.index,
    ...
    st.caption("Made for 3 Friends | Dynamic Stock Analysis")
